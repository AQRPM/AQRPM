...
pub use aead::{KEYBYTES as KEYLEN, NONCEBYTES as NONCELEN};
pub use pwhash::SALTBYTES as SALTLEN;
...
pub struct Vault {
    ...
    salt: [u8; SALTLEN],
    entries: Vec<([u8; NONCELEN], Vec<u8>)>
}
#[derive(Deserialize, Serialize, PartialEq, Eq)]
pub struct Entry {
    name: String,
...
pub fn pwhash(password: &[u8], salt: &pwhash::Salt) -> Result<[u8; KEYLEN], EncryptError> {
    let mut key = [0; KEYLEN];
...
impl Vault {
    ...
    pub fn entry_decrypt(&self, index: usize) -> Result<Entry, Error> {
        ...
        let key   = aead::Key::from_slice(&self.key).expect("Key is not the right length. See aqrpm::KEYLEN.");
    ...
    pub fn entry_encrypt_with_key(&self, entry: &Entry, key: &[u8]) -> Result<([u8; NONCELEN], Vec<u8>), Error> {
        let key   = aead::Key::from_slice(&key).expect("Key is not the right length. See aqrpm::KEYLEN.");
    ...
    pub fn entry_encrypt(&self, entry: &Entry) -> Result<([u8; NONCELEN], Vec<u8>), Error> {
